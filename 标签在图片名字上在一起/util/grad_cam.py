...
def superimpose(input_img, heatmap):
    ...
    cv2.normalize(eyeheatmap,eyeheatmap , 0, 255, cv2.NORM_MINMAX)
    cv2.normalize(faceheatmap,faceheatmap , 0, 255, cv2.NORM_MINMAX)
    heatmap =np.concatenate((eyeheatmap,faceheatmap),axis = 0)
    print("debug",eyeheatmap.shape,faceheatmap.shape)
    print("debug2",heatmap)
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)#生成热力图
